...
def cum4_diagsWHITE(Z0,U0):
    """Compute the diagonal entries of a transformed cumulant tensor implicitly

    Parameters
    ----------
    Z0 : double
        Data of which the fourth-order cumulant must be computed.

    U0 : double
        Transformation to apply in all modes
    """
...
def measdiag4WHITE(T0,Z0,U0):
    """Compute the measure of diagonality of a transformed cumulant tensor implicitly in comparison to the base tensor. This only works if U0 is orthonormal.
...
def cum4tensor(X):
    """Compute the cumulant tensor

    Parameters
    ----------
    X : double
        Data of which to compute the cumulant tensor
    """
...
def mom4tensor(X):
    """Compute the fourth-order moment tensor

    Parameters
    ----------
    X : double
        Data of which to compute the moment tensor
    """
...
def cum4tensor2(X):
    """Compute the cumulant tensor. Faster than version 1
...
def cum4tensor3(X):
    """Compute the cumulant tensor. Faster than version 1 and version 2.
...
def cum4tensor4(X):
    """Compute the cumulant tensor in sicewise fashion similar to how the implicit HOSVD algorithm does.
...
def whitendataSVD(X,removemean):
    """Produce whitened data for ICA use. Additionally remove the mean.

    Parameters
    ----------
    X : double
        Data which to whiten
...
def SVDinit(X,R,removemean):
    """Prepare data for ICA use by whitenening, scaling and optionally removing mean.

    Parameters
    ----------
    X : double
        Data which to prepare.

    R : int
        Number of components to keep after whitening.
...
def testset(n,I,noise_factor,start_end=[0,8],random_mixmatrix=True):
    """Create a test Blind Source Separation data-set. Choice of 2 to 4 latent source components. Choosing more than 4 will result in additional mixtures with only 4 source components.

    Parameters
    ----------
    n : int
        Number of components/observations. Range 1 to 4 will result in equal amount of observations with en equal amount of latent source signals present. More than 4 results in additional mixtures of 4 latent components.
    ...
    start_end : array of shape (1,2) double
        Start time and end time of the signals.

    random_mixmatrix : Boolean
        Randomize the mixing matrix from a uniform distribution.
    """
...
def Errorfunc(S,S_hat):
    """Evaluate the estimation results of an ICA algorithm. The function sorts the estimated signals in order of the true source signals and corrects them for sign too.

    Parameters
    ----------
    S : double
        Data of source signals
...
def testset3d(I,noise_factorX,noise_factorY,noise_factorZ,mixdimP):
    ...
    noise_factorX : double
        Standard devation of noise along X-axis

    noise_factorY : double
        Standard deviation of noise along Y-axis.

    noise_factorZ : double
        Standard deviation of noise along Z-axis.

    mixdimP : int
        Dimensions of mixing matrix along each mode.
    """
...
def voxelizer(Sx,Sy,Sz,Xdim,fscale):
    """Voxelize a set of 3d signals.

    Parameters
    ----------
    Sx : int
        Max range along X-axis.

    Sy : int
        Max range along y-axis.

    Sz : int
        Max range along Z-axis.

    Xdim : int
        Number of voxels along the X-axis.

    fscale : double
        Scaling coefficient for the 3d signals.
    """
...
def offdiag4(T):
    """Compute the offdiagonal norm of a 4-way tensor.

    Parameters
    ----------
    T : 4-way array, double
        Tensor.
    """
    I = T.shape[0]
    C = np.copy(T)
...
def diag4(T):
    """Compute the diagonal norm of a 4-way tensor.
...
def trace4D(T):
    """Compute the diagonal entries of a 4-way tensor.
...
def offdiag3(T):
    """Compute the offdiagonal norm of a 3-way tensor.

    Parameters
    ----------
    T : 3-way array, double
        Tensor.
    """
    I = T.shape[0]
    C = np.copy(T)
...
def diag3(T):
    """Compute the diagonal norm of a 3-way tensor.
...
def measdiag4(T0):
    """Compute the measure of diagonality of a 4-way tensor.

    Parameters
    ----------
    T0 : 4-way array, double
        Tensor.
    """
    T = np.copy(T0)
...
def opt_jacobi_rot4_A(T,p,q):
    """Compute the optial jacobi rotation for the COM2 algorithm.
...
def COM2_own(Xmix,tol,U0,n_iter,print_f):
    ...
    U0 : array, double
        Initial unmixing matrix estimate.
...
def IMPCPDfunOPT(lamU,X,alpha,R):
    """Implicit CPD gradient based first-order optimization algorithm. Produces the cost function value and the gradients such that it can be used with any standard Python solver.

    Parameters
    ----------
    lamU : vector, double
        Vectorize gradients stacked on top of each other. 

    X : array, double
        Data of which to compute the cumulant tensor CPD.

    alpha : double
        Hyperparameter with which to set the ground value of the cost function.

    R : int
        Number of components to estmimate.
    """
    P = X.shape[0]
...
# Implicit CPD algorithm for fixed point iteration     
def IMPCPDfunFixedPoint(X,lam,U,alpha,R,IMPCPD_or_fastICA,WHITE=True):
    """Implicit CPD fixed-point optimization algorithm. Produces the cost function value and the new solution estimates. 
        Can be changed such that it equals FastICA.
    ...
    lam : vector, double
        Initial estimate of the weights.

    U : array, double,
        Initial estimate of the factor matrix.
    ...
    IMPCPD_or_fastICA : Boolean
        Whether to use FastICA or implicit CPD.

    WHITE : Boolean
        Whether data is white or not.
    """
    P,I = X.shape
...
# full implicit CPD fixed-point symmetric solution      -- OLD
def symIMPCPDff(X,lamff,Uff,alphaff,Rff,n_iter,IMPCPD_or_fastICA,symorth,f_stopcrit,WHITE=True):
    """Wrapper for the implicit CPD fixed-point optimization algorithm.


    !!! OLD !!!

    Newer and better versions can be found below.
    ...
    symorth : Boolean
        Whether to perform symmetric orthogonalization in between iterations.

    f_stopcrit : Boolean
        Whether to use a stopping criterion.
...
def symIMPCPDffWHITEFAST(X,lamff0,Uff0,Rff,n_iter,tol):
    """Implicit CPD fixed-point optimization algorithm. Exploits simplifications due to whitened data which makes it faster. Does not compute cost CP cost function.
...
def QRT_fulltensor(T0,R,n_repeat):
    """QR-Tensor (QRT) algorithm for a symemtric 4-way tensor. Moodified for ICA.

    Devised by: Pierre-Antoine Denarié, 2022, for master thesis of Systems and Control TU Delft.
    Parameters
    ----------
    T0 : 4-way array, double
        Tensor which to decompose.
...
def c4mat_uniq(Z):
    """Compute only the unique slices of the cumulant tensor.

    Devised by: Pierre-Antoine Denarié, 2022, for master thesis of Systems and Control TU Delft.
    Parameters
    ----------
    Z : array, double
        Data of which to compute the cumulant tensor slices.
    """
    P,I = Z.shape
...
def idxc4mat_uniq(P):
    """Function which uses a polynomial to compute all indices of unique cumulant tensor slices.

    Devised by: Pierre-Antoine Denarié, 2022, for master thesis of Systems and Control TU Delft.
    Parameters
    ----------
    P : int
        Size of each mode of the cumulant tensor.
    """
    Puniq = 0
    for i in range(1,P+1):
        Puniq += i*(P+1-i)
...
def QRT(Z0,R,Q0,n_repeat=10,tol=10**(-6),V_error='fastICA',Method='parallel'):
    """Implicit QR-Tensor (QRT) algorithm for the cumulant tensor for Independent Component Analysis.
        Exploits whiteness and the fact that only the QR decomposition of the fibers along the superdiagonal have to be computed to speed up the process. 
        Furthermore, it allows for varying convergence criterions and allows for a deflationary and parallel computation method.

    Devised by: Pierre-Antoine Denarié, 2022, for master thesis of Systems and Control TU Delft.
    Parameters
    ----------
    Z0 : array, double
        Data of which the cumulant tensor must be implicitly decomposed.

    R : int
        Number of components to estimate.

    Q0 : array, double
        Initial estimate of the factor matrix Q.

    n_repeat : int
        Number of iterations to perform. For the parallel scheme this is the total amount of iterations whereas for the deflationary scheme this is the amount of iterations 
        that is performed per deflated component.

    tol : double,
        Convergence tolerance.

    V_error : {'fastICA',''as_identity','abs_difference'}, string
        Convergence criterion.

    Method : {'parallel','deflationary'}, string
        Whether to use a deflationary or parallel computation scheme.
    """
    Z = Q0 @ np.copy(Z0)
    P = Z.shape[0]
    I = Z.shape[1]
...
# Create alternating QR tensor function with iplicit use of the cumulant tensor, straight from data
def Implicit_QRT_FULL(Z,R,n_repeat,tol):
    """Full implicit QRT algorithm. COmputes the unnecessary QR decomposition of the offdiagonal fibers too.
...
def QRT_FinalFASTWHITE(Z0,R,Q0,n_repeat=10,tol=10**(-6)):
    """Fast parallel implicit QRT for whitened data without additioanl overhead cost from main QRT. Meant for performance testing. Uses the FastICA convergence criterion.
    ...
    Q0 : array, double
        Initial factor matrix estimate.
...
def QRT_FinalFAST(Z0,R,Q0,n_repeat=10,tol=10**(-6)):
    """Fast parallel implicit QRT for non-whitened data without additional overhead cost from main QRT. Meant for performance testing. Uses the FastICA convergence criterion.
...
## Kim Batselier QRST algorithm
def QRST4(T0,tol,kmax):
    """QR-Symmetric Tensor algorithm for 4-way tensors to compute tensor eigenpairs.

    Batselier, K., & Wong, N. (2014). A QR Algorithm for Symmetric Tensors. arXiv preprint arXiv:1411.1926.

    Parameters
    ----------
    T0 : 4-way array, double
        Tensor which to compute tensor eigenpairs from.
...
def CPD_gevd(T0,R):
    """CP-GEVD algorithm presented ni reference below. Ported over from the Matlab implementation. Computes the CP decomposition of a tensor determinstically using Generalized Eigenvalue
    Decomposition.

    Sanchez, E., & Kowalski, B. R. (1990). Tensorial resolution: a direct trilinear decomposition. Journal of Chemometrics, 4(1), 29-45.

    Ported over by: Pierre-Antoine Denarié, 2022, for master thesis of Systems and Control TU Delft.
    Parameters
    ----------
    T0 : N-way array, double
        Tensor which to decompose.
...
def HOSVD_iterFINAL(Zorig,Core_only=False,method=2,reorth_step=0):
    """Implicit HOSVD of the fourth-order cumulant tensor. Allows for its computation while not exceeding O(P^2) in storage. Uses an SVD-update scheme. Implementation is taken from:
    ...
    ----------
    Zorig : array, double
        Data from which the implicit HOSVD must be computed.

    Core_only : {False, True,'HALF'}
        Whether to use all slices (False), use only the unique slices once ('HALF') or use only the core slices ('True').

    method : int
        Root finding procedure selection used by the SVD-updater.

    reorth_step : int
        Number of reorthogonalization steps performed by the SVD-updater.
    """
    Z = np.copy(Zorig)
    P,I = Z.shape
...
def HOSVD_iterFINAL_CPDGEVD(Zorig,R,Core_only=False,method=2,reorth_step=0):
    """Implicit HOSVD of the fourth-order cumulant tensor for the implicit CP-GEVD algorithm. Additionally passes the computed tensor slices.
    Allows for its computation while not exceeding O(P^2) in storage. Uses an SVD-update scheme. Implementation is taken from:
...
def CPD_gevdIMPLICIT_FINAL(Z,R,version,SVDCore_only=False,method=2):
    """Implicit CP-GEVD of the fourth-order cumulant tensor. 

    Devised by: Pierre-Antoine Denarié, 2022, for master thesis of Systems and Control TU Delft.
    Parameters
    ----------
    Z : array, double
        Data from which the implicit deterministic CP must be computed.

    R : int
        Tensor rank. ALternatively this is the amount of components to estimate.

    version : int
        Version of the algorithm to use. The old method (version=1) summs up the slices for inwards projection use whereas version=0 uses the core slices.


    SVDCore_only : {False, True,'HALF'}
        Whether to use all slices (False), use only the unique slices once ('HALF') or use only the core slices ('True').
...
def CPD_gevdIMPLICIT_FINAL_ORTHO(Z,R,version):
    """Implicit CP-GEVD of the fourth-order cumulant tensor which additionally orthogonalizes the first factor matrix. 
...
def CPDerror(T,lam,DU,symmetric):
    """Function to compute the CP-error of a CP-decomposition. The error is taken as the norm of the difference over the norm of the original tensor.
    ...
    lam : vector, double
        Weights of the CP decomposition.

    DU : dictionary / array, double
        If symmeric is False then DU is a dictionary containing all of the factor matrices of the decomposition in correct order.
        If symmetric is True then DU is the single factor matrix of the decomposition.

    symmetric : Boolean
        Whether the CP decomposition is symmetric or not.
