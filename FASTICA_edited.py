...
def _ica_def(X, tol, g, fun_args, max_iter, w_init):
    ...
    n_iter = []
    lim = np.zeros([n_components,max_iter])
    # j is the index of the extracted component
    for j in range(n_components):
        w = w_init[j, :].copy()
        w /= np.sqrt((w ** 2).sum())

        lim_temp = np.zeros(max_iter)
        for i in range(max_iter):
            ...
            lim_temp[i] = np.abs(np.abs((w1 * w).sum()) - 1)
            w = w1
            if lim_temp[i] < tol:
                break
...
def _ica_par(X, tol, g, fun_args, max_iter, w_init):
    ...
    for ii in range(max_iter):
        ...
        lim[ii] = max(abs(abs(np.diag(np.dot(W1, W.T))) - 1))
        W = W1
        if lim[ii] < tol:
            break
    # else:
