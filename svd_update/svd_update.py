...
class SVD_updater(object):
    ...
    def add_column(self, new_col, same_subspace=None, method=2):
        """
...
#            Vh = np.dot(V1.T,Vh) # V matrix. Need to return V.T though
            ...
            U1, self.S, V1 = _SVD_upd_diag( self.S, m_vec, new_col=False,method=method)
...
def _arrays_merge(a1, a2, decreasing=True , func=None):
    ...
    inds_a1 = [*range(len_a1)]; inds_a2 = [*range(len_a1, len_a1+len_a2)] # indices of array elements                         #### EDITED THIS MYSELF, ADDED THE BRACKETS AND * TO THE RANGES
...
def _SVD_upd_diag(sigmas, m_vec, new_col=True,method=2):
    ...
    for i in range(0, len(new_sigmas) ):
...
def test_update_svd(n_rows,start_n_col, n_max_cols, step_n_col):
    ...
    column_sizes = []; k = 0
    for column_num in range(start_n_col,n_max_cols+1,step_n_col):
        k += 1
...
def test_SVD_comp_complexity(n_rows,start_n_col, n_max_cols, step_n_col):
    ...
    iter_counter = 0
    for ii in range( 0,n_max_cols,step_n_col ):
...
#    res = test_SVD_update_reorth(1000, 1000, 1500, 0.05, 2000, '1000_1000_no_reorth_10.mat')
    ...
    for i in range(it_start, it_len+1): # find all singular except the last
        res = sp.linalg.lapack.dlasd4(i, sgm, mvc)
        roots.append(res[1])
